...
const Validar=(x)=>{
    switch(x.target.name){
        ...
        break;
        case "Promedio":
            validarCampo(expre.prom,x.target,'Promedio');
        break;
        case "fechanac":
...
function ValidarFormula(){
    ...
    if(campoval.nombre && campoval.apellido1 && campoval.apellido2 && campoval.curp && campoval.Num && campoval.CP && campoval.telefono && campoval.correo && campoval.boleta && campoval.fechanac && campoval.Promedio){
        ...
        if(mun===""){
            console.log('error seleccionar municipio.');
            alert("Por favor, coloque su municipio.");
            document.getElementById('form_mensaje').classList.add('form_mensaje-activo');
            return false; 
        }
        else{
